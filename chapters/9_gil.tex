...
\begin{quote}
     \ldots . its grammar relates the semantic structure of sentences directly to linear order and
     ...
     Modifier precedes (or follows) Modified.  None of these requires syntax  \ldots .
\itdopt{What do the dots mean? Are they ommissions in quoting? If so, dots at the beginning and the
  end should be dropped. Dots should be replaced by ``[\ldots]'' unless the literal text contained
  the dots.}
\end{quote}
...
\citet{pawlk}\itdopt{reference missing} — from whom I adapted the above term — refers to ``idiomatic competence'',
