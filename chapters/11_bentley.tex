...
\ea	\label{bentley_example_1a} \textbfemph{\textsc{+\textsc{se}}} - \textit{sparpagliare} ‘scatter’ (tr.) ~ \textit{sparpagliarsi} ‘scatter’ (intr.) 	(Italian) \\

\ex	\label{bentley_example_1b} 
\textbfemph{\textsc{±\textsc{se}}} - \textit{riscaldare} ‘heat’ (tr.) ~ \textit{riscaldare-riscaldarsi} ‘heat’ (intr.)  \\

\ex \label{bentley_example_1c}	
\textbfemph{\textsc{-\textsc{se}}} - \textit{aumentare} ‘increase’ (tr.) ~ \textit{aumentare} ‘increase’ (intr.)  \\
\z
\z
...
The article is organized as follows. After a brief overview of the existing approaches to anticausative \textsc{se} and their predictions (section \sectref{bentley_section_2}), we introduce our findings (section \sectref{bentley_section_3}—section \sectref{bentley_section_4}). We then provide our parallel architecture account of the causative alternation in Italian (section \sectref{bentley_section_5}) and draw some conclusions on the theoretical significance of our results and analyses (section \sectref{bentley_section_6}). 
...
\ea \label{bentley_example_3}
    ...
    {}.\\
    \gll si \textbfemph{può} \textbfemph{sgonfiare} \textit{per} \textit{essere} \textit{riposta} \ldots  \\
...
\ea \label{bentley_example_4}
    ...
    \gll Lui entra in bagno, \textbfemph{si} \textbfemph{pettina}, \textbfemph{si} \textbfemph{spettina} \ldots  \\
...
\gll  \ldots  quanto costa \textbfemph{riscaldarsi} con un caminetto a legna. \\
...
Since in most cases an anticausative reading - i.e., a reading with no coreferentiality - could not be ruled out, we made the methodological decision not to leave out the putative intransitive reflexives from the +\textsc{se} intransitive counts. We did, however, return to the distinction between the two structures in the qualitative analysis (section \sectref{bentley_section_3.2}).
...
Figure \figref{fig:bentley_figure_1} shows the proportion of ±\textsc{se} tokens in the non-passive intransitive subset of each verb in the sample. The relevant evidence is given in blue (+\textsc{se} non-passive intransitives) and orange (relevant -\textsc{se} intransitives), whereas we represent in grey the -\textsc{se} intransitives with a covert causee (cf. \REF{bentley_example_6}), which will be disregarded in the analysis.
...
To be sure, the incidence of \textsc{se} varied considerably, yielding proportions below 10\% with \textit{cuocere} ‘cook’ and \textit{migliorare} ‘improve’ and above 90\% with \textit{sbriciolare} ‘crumble’, \textit{sgonfiare} ‘deflate’, and the sole verb patterning exclusively as +\textsc{se}, namely \textit{sparpagliare} ‘scatter’ (the very few -\textsc{se} tokens of this verb were of the irrelevant type, section \sectref{bentley_section_3.1}). The low \textsc{se} proportions with \textit{cuocere} ‘cook’ depend on the frequent occurrence of this verb in culinary recipes: 71.8\% of the -\textsc{se} non-passive intransitives are irrelevant (cf. \REF{bentley_example_6}), while only 20.5\% are anticausatives (see Figure \figref{fig:bentley_figure_1} and section \sectref{bentley_section_3.1}).

As for \textit{migliorare} ‘improve’, despite first appearances it turned out to be a -\textsc{se} verb, in accordance with our expectations (see Table \tabref{tab:bentley_table_1}). Indeed, only one of its +\textsc{se} tokens arguably lent itself to an anticausative analysis (cf. \REF{bentley_example_7a}), the others being reflexive, as suggested by the replaceability of atonic \textsc{se} in \REF{bentley_example_7b} with the tonic reflexive sé stesso ( \ldots  \textit{per migliorare me stesso} ‘to improve myself/my performance’) (section \sectref{bentley_section_3.1}).
...
The existence of alternating verbs which never exhibit \textsc{se} in the intransitive is not a novel result (section \sectref{bentley_section_1}). The purpose of this section is to identify which verbs from our sample belong in this group and to discuss the properties which set them apart from the alternating verbs that admit \textsc{se} marking. In Figure \figref{fig:bentley_figure_2} we show the proportions of transitive hits obtained for each verb.
...
Observation of Table \tabref{tab:bentley_table_2} and Figure \figref{fig:bentley_figure_2} suggests that \textit{aumentare} ‘increase’ is an alternating   -\textsc{se} verb. We provide an intransitive and, respectively, a transitive example of this verb here.

\hspace*{\fill}(itTenTen20, 29/10/2022)\quad

\ea \label{bentley_example_8}
    \gll  \ldots  i casi di tumore del pancreas sono \textbfemph{aumentati} del 59\%  \ldots  \\
...
\ea \label{bentley_example_10}
    \gll La	crisi		economica	avrebbe 							\textbfemph{fatto} \textbfemph{aumentare} l’ {incidenza \ldots }\\
...
\ea \label{bentley_example_11}
    ...
    \gll In	due	giorni	ho						\textbfemph{ricamato}						un cuscinetto \ldots  \\
    ...
    \gll In	due	giorni	ho						\textbfemph{fatto}						\textbfemph{ricamare}					un	cuscinetto \ldots  \\
...
\textit{Migliorare} ‘improve’ can also be classified as an alternating -\textsc{se} verb, despite the findings reported in Table \tabref{tab:bentley_table_2}, because its \textsc{se} marked intransitive tokens are mostly reflexive (section \sectref{bentley_section_3.2}, cf. \REF{bentley_example_7b}). We should also note that this verb has the highest transitive percentages (see Figure \figref{fig:bentley_figure_2}) and the transitive tokens pass the causativity test mentioned above, as suggested by the synonymity of \REF{bentley_example_12a} with its constructed paraphrase in \REF{bentley_example_12b}.
...
In Table \tabref{tab:bentley_table_3} we show the results on transitivity and anticausative marking, highlighting the two alternating -\textsc{se} verbs. (In the next section, we shall explain why the other -\textsc{se} verbs, \textit{marcire} ‘rot’ and \textit{sbocciare} ‘blossom’, are classified as -transitive.)
...
In section \sectref{bentley_section_5.4.1} we shall argue that a constructional constraint rules out \textsc{se} anticausativization in Italian, when the conditions on direct causation are not satisfied. In this perspective, the verbs discussed here do not challenge the argument realization analyses of \textsc{se} anticausativization, after all.
...
Unlike \textit{aumentare} ‘increase’ and \textit{migliorare} ‘improve’, \textit{marcire} ‘rot’ and \textit{sbocciare} ‘blossom’ were classified as non-transitivizing because their transitive hits amounted to less than 1\% of their overall attestations (see Appendix 2). These results are highlighted in Table \tabref{tab:bentley_table_4}.
...
\ea \label{bentley_example_16}
    \gll { \ldots  il malcostume  \ldots   abbia guastato e} \textbfemph{marcito}	la			questione	meridionale \ldots   \\
...
\ea \label{bentley_example_24}
    ...
    \gll  {\ldots}  lo chassis  {\ldots}  che		già 				\textbfemph{sta} 			\textbfemph{asciugando}	in 	terrazzo. {(itTenTen20, 30/08/2022)}\\
...
\ea \label{bentley_example_25}
    ...
    \gll Dopo che	si 		\textbfemph{sono} 		\textbfemph{cotti},  {\ldots}  tagliarli			a fettine. {(itTenTen20, 23/08/2022)} \\
    ...
    \gll Quando		i		capelli	si		\textbfemph{sono} 		\textbfemph{asciugati}  {\ldots}  appaiono		come “incollati”. {(itTenTen20, 30/08/2022)}\\
    ...
    \gll  {\ldots}  i			mammut,		mentre		mangiavano  {\ldots}  si 		\textbfemph{sono} 		\textbfemph{congelati} 		all’istante. {(itTenTen20, 02/09/2022)} \\
...
\ea \label{bentley_example_27}
    \gll {Bisogna riportare il disegno  \ldots  e lasciare}  che		\textbfemph{si}		\textbfemph{asciughi}					\textit{per}		\textit{circa}	2	\textit{giorni}. \\
...
\ea \label{bentley_example_37}
    \gll  {\ldots}  un dramma: il		dolce \textbfemph{si} \textbfemph{	è }					\textbfemph{bruciato}. \\
    	{} a			tragedy		the	cake		\textsc{se}		be.3\textsc{sg}	burn.\textsc{ptcp} \\
    \glt 	‘a tragedy: the cake burned.’
\z

\ea \label{bentley_example_38}
    \gll { \ldots   un ferro da stiro acceso   \ldots } molti di		noi	lo	hanno			toccato				e				\textbfemph{si}	\textbfemph{sono}			\textbfemph{bruciati}. \\
...
\ea \label{bentley_example_39}
    \gll  {\ldots}  i			vostri		sentimenti		\textbfemph{si}		\textbfemph{erano} 					\textbfemph{congelati}			senza			motivo.  \\
...
\ea \label{bentley_example_40}
    \gll  {\ldots}  la		scrittura		\textbfemph{si}		\textbfemph{asciuga}.  \\
...
In such cases the cause seems to us not to be at issue (see the addition of ‘for no reason’ in 39) and the effect of the change on the causee to be in focus. We return to this point in section \sectref{bentley_section_5.4.1}.
...
The representation in \REF{bentley_example_43} includes the defining components of accomplishment LSs: a process (PROC \textbfemph{becoming.higher.on.[blossom]scale}´ (x)) and a specific result state (FIN\textbfemph{ blossomed}´ (x)). The concomitance symbol ∧ indicates that the result state is the final stage of the process. 
...
That \textit{arrugginire} ‘rust’ is a degree achievement is suggested, among the other diagnostics introduced in section \sectref{bentley_section_3.3.1}, by its compatibility with expressions of measure like ‘a little’. 


\ea \label{bentley_example_45}
    \gll
     {\ldots}  quel		campanello  {\ldots} 		Forse			\textbfemph{si} 		\textbfemph{era} 							un	po’ 		\textbfemph{arrugginito}  {\ldots}  \\
...
The two LSs in (44a-b) differ in that the latter includes the representation of a cause, [\textbfemph{do}´ (x, [Ø])]. The argument of [do´ (x,  \ldots )] is an effector \citep{vanvalin1996case}, which means that the verb is not lexically agentive. Indeed, it takes inanimate transitive subjects. 

\hspace*{\fill}(itTenTen20, 24/08/2022)\quad
\ea \label{bentley_example_46}
    \gll {Le impurità dell’acqua e l’ossigeno in eccesso } potevano  {\ldots } \textbfemph{arrugginirli}. \\
    {} can.\textsc{pst}.3\textsc{pl}			rust.\textsc{inf}.them \\
    \glt
    ‘The impurities in the water and excessive oxygen could rust them.’
\z

In addition, the transitive of \textit{arrugginire} ‘rust’ can be paraphrased with a periphrastic fare ‘make’ causative, which is further evidence against an agentive analysis (section \sectref{bentley_section_3.3.1}). The symbol Ø in the square brackets in \REF{bentley_example_44b} indicates that the lexicon does not specify exactly how the cause brings about the change.
...
Drawing on the causer suppression analyses (section \sectref{bentley_section_2}), we treat anticausativization as a monotonic
...
The representation in Figure \figref{fig:bentley_figure_3} includes a step of the linking, macrorole assignment, which is important to grasp generalizations that other frameworks formalize in terms of argument structure. There are only two macroroles, actor and undergoer, which are assigned in accordance with explicit principles grounded in the hierarchical relations between the five argument positions that are possible in Logical Structure \citep[82—198]{vanvalin1997syntax}. The overt argument of Figure \figref{fig:bentley_figure_3} (\textit{campanello} ‘doorbell’) is an undergoer because it is the lower candidate for macrorolehood of a two-place predicate. The status of the subject as the lower argument in logical structure will turn out to be relevant to \textsc{se} marking. 
...
Causer suppression can be marked by the morpheme \textsc{se}, in which case anticausativization is overt. To understand the rationale of this marking it is necessary to consider the other domains where \textsc{se} figures systematically, namely passives, impersonals and reflexives (section \sectref{bentley_section_1}, section \sectref{bentley_section_3.1}). In all cases, \textsc{se} marks the suppression of the highest-ranking argument in the semantic representation of the clause. Consider the \textsc{se} passive in \REF{bentley_example_51a}, with its semantic representation in \REF{bentley_example_51b}, and the \textsc{se} impersonal in \REF{bentley_example_52a}, with its representation in \REF{bentley_example_52b}.
...
\gll {Bisogna riportare il disegno  \ldots  e lasciare} che		si		asciughi					per		circa	2	giorni. \\
	{} that	\textsc{se}		dry.\textsc{sbjv}.3\textsc{sg}		for		circa	2	days \\
\glt 			‘One needs to take the drawing back \ldots  and let it dry for about two days.’
\z
...
the inferences that arise from such marking. The evidence was discussed in section \sectref{bentley_section_4.2}; consider \REF{bentley_example_59}, which is an abridged version of \REF{bentley_example_31}. 
