...
\glt `The oak-wood burned down (*for days).’ \citep[401]{cennamo2012aspectual}
\z
...
\ex \label{bentley_example_44b}
    $[\textit{do´} (x, [\emptyset])] CAU\textsc{se} PROC \textit{becoming.higher.on.[rust]scale´ }(y) ∧ FIN \textit{rusted.by.some.amount´ }(y)$     
\z
\z
...
The two LSs in (44a-b) differ in that the latter includes the representation of a cause, [\textit{do}´ (x, [∅])]. The argument of [do´ (x,  \ldots )] is an effector \citep{vanvalin1996case}, which means that the verb is not lexically agentive. Indeed, it takes inanimate transitive subjects. 
...
\ea \label{bentley_example_47}
    $\lbrack$ \textit{do}´ (x, $\lbrack \emptyset \rbrack$ ) $\rbrack$ CAU\textsc{se} INGR \textit{broken}´ (y)
\z
...
\ea  \label{bentley_example_50}
    $\lbrack$ \textit{do}´ ($\emptyset, \lbrack \emptyset \rbrack$ ) $\rbrack$ \textit{CAU\textsc{se} PROC becoming.higher. $\lbrack$ rust $\rbrack$ scale}´ (campanello) ∧ FIN \textit{rusted.by.some.amount}´ (campanello)
\z
...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{figures/bentley_figure4.png}
\caption{}
