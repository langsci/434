...
Italian\\
\ea	\label{bentley_example_1a} \textbfemph{\textsc{+\textsc{se}}} - \textit{sparpagliare} ‘scatter’ (tr.) ~ \textit{sparpagliarsi} ‘scatter’ (intr.) \\
...
The article is organized as follows. After a brief overview of the existing approaches to anticausative \textsc{se} and their predictions (\sectref{bentley_section_2}), we introduce our findings (\sectref{bentley_section_3}--\sectref{bentley_section_4}). We then provide our parallel architecture account of the causative alternation in Italian (\sectref{bentley_section_5}) and draw some conclusions on the theoretical significance of our results and analyses (\sectref{bentley_section_6}). 
...
\ea (itTenTen20, 02/09/2022)\label{bentley_example_3}
    ...
    \gll {La lanterna LED  \ldots  in previsione di un periodo di inutilizzo} si \textbfemph{può} \textbfemph{sgonfiare} \textit{per} \textit{essere} \textit{riposta} \ldots  \\
...
Since in most cases an anticausative reading - i.e., a reading with no coreferentiality - could not be ruled out, we made the methodological decision not to leave out the putative intransitive reflexives from the +\textsc{se} intransitive counts. We did, however, return to the distinction between the two structures in the qualitative analysis (\sectref{bentley_section_3.2}).
...
As for \textit{migliorare} ‘improve’, despite first appearances it turned out to be a -\textsc{se} verb, in accordance with our expectations (see \tabref{tab:bentley_table_1}). Indeed, only one of its +\textsc{se} tokens arguably lent itself to an anticausative analysis (cf. \REF{bentley_example_7a}), the others being reflexive, as suggested by the replaceability of atonic \textsc{se} in \REF{bentley_example_7b} with the tonic reflexive sé stesso ( \ldots  \textit{per migliorare me stesso} ‘to improve myself/my performance’) (\sectref{bentley_section_3.1}).
...
The existence of alternating verbs which never exhibit \textsc{se} in the intransitive is not a novel result (\sectref{bentley_section_1}). The purpose of this section is to identify which verbs from our sample belong in this group and to discuss the properties which set them apart from the alternating verbs that admit \textsc{se} marking. In \figref{fig:bentley_figure_2} we show the proportions of transitive hits obtained for each verb.
...
Observation of \tabref{tab:bentley_table_2} and \figref{fig:bentley_figure_2} suggests that \textit{aumentare} ‘increase’ is an alternating   -\textsc{se} verb. We provide an intransitive and, respectively, a transitive example of this verb here.
...
In \tabref{tab:bentley_table_3} we show the results on transitivity and anticausative marking, highlighting the two alternating -\textsc{se} verbs. (In the next section, we shall explain why the other -\textsc{se} verbs, \textit{marcire} ‘rot’ and \textit{sbocciare} ‘blossom’, are classified as -transitive.)
...
Unlike \textit{aumentare} ‘increase’ and \textit{migliorare} ‘improve’, \textit{marcire} ‘rot’ and \textit{sbocciare} ‘blossom’ were classified as non-transitivizing because their transitive hits amounted to less than 1\% of their overall attestations (see Appendix 2). These results are highlighted in \tabref{tab:bentley_table_4}.
...
\ea \label{bentley_example_24}
    ...
    \gll Mentre	le			uova	\textbfemph{cuociono}, { prepariamo la salsa olandese.} \\
    ...
    \gll  {\ldots}  lo chassis  {\ldots}  che		già 				\textbfemph{sta} 			\textbfemph{asciugando}	in 	terrazzo.\\
    ...
    \gll Infilare			nel			Freezer	a		\textbfemph{congelare}		per 	una	mezzoretta.\\
...
\ea \label{bentley_example_25}
    ...
    \gll Dopo che	si 		\textbfemph{sono} 		\textbfemph{cotti},  {\ldots}  tagliarli			a fettine. \\
    ...
    \gll Quando		i		capelli	si		\textbfemph{sono} 		\textbfemph{asciugati}  {\ldots}  appaiono		come “incollati”. \\
    ...
    \gll  {\ldots}  i			mammut,		mentre		mangiavano  {\ldots}  si 		\textbfemph{sono} 		\textbfemph{congelati} 		all’istante.  \\
...
In such cases the cause seems to us not to be at issue (see the addition of ‘for no reason’ in 39) and the effect of the change on the causee to be in focus. We return to this point in \sectref{bentley_section_5.4.1}.
...
The two LSs in (44a-b) differ in that the latter includes the representation of a cause, [\textbfemph{do}´ (x, [∅])]. The argument of [do´ (x,  \ldots )] is an effector \citep{vanvalin1996case}, which means that the verb is not lexically agentive. Indeed, it takes inanimate transitive subjects. 
...
Drawing on the causer suppression analyses (\sectref{bentley_section_2}), we treat anticausativization as a monotonic
