...
% Pullum comments:  Suppose there are finite clauses (S[fin]) and infinitival clauses (S[inf]), and we take them to be different syntactic categories. If you can embed an S[inf] in an S[fin], then that is embedding, but not self-embedding. It doesn't necessarily give you a way of constructing sentences of arbitrary length.

% On the other hand, if an S[fin] can be embedded in another S[fin], that's self-embedding. It should yield sentences of arbitrary length, provided only that there are strings of words that can be the yield of an S[fin] (i.e., provided S[fin] is not a useless category).

% If you can embed an S[fin] in an S[inf] but not in an S[fin], then you have indirect self-embedding: an S[fin]. That too can yield arbitrarily long sentences.

English has syntactic self-embedding in many structures, such as in  possessives \exref{syn_self_embed_eng1} (where an NP expands to another NP);   clausal complements \exref{syn_self_embed_eng2} (where an S expands to another S further down the grammar), or coordination as in \exref{syn_self_embed_eng3}.

\begin{myexample}
\label{syn_self_embed_eng}
\begin{examples}
\item \label{syn_self_embed_eng1} Possessives: NP → NP ’s N;  NP → N \newline
[$_{NP}$ [$_{NP}$ [$_{NP}$ [$_{NP}$ John]’s mother]’s brother]’s house]
\item \label{syn_self_embed_eng2} Clausal complements: S → NP  VP;  VP → V CP;  CP → Comp S\newline
[$_{S}$ John thinks that [$_{S}$ Mary said that [$_{S}$ the girl cried]]]
\item \label{syn_self_embed_eng3} Coordination: S → S Conj S; NP → NP Conj NP;  VP → VP Conj VP \newline
[$_{NP}$ [$_{NP}$ [$_{NP}$ John] and [$_{NP}$ Mary]] and [$_{NP}$ Bill]]
\end{examples}
\end{myexample}
...
A context-free grammar consists of a tuple (T, N, S, R), such that:

\begin{itemize}
    \item T is a finite set of terminal symbols: the words, that make up the lexicon or dictionary
    \item N is a finite set of non-terminal symbols
    \item S $\epsilon$ N is the start symbol
    \item R is a finite set of rules of the form X → $\alpha$, where X $\epsilon$ N, and $\alpha$ is a sequence of symbols drawn from V or N
\end{itemize}
...
\begin{examples}
\item \label{corpus1a} The girl saw the dog.
\item \label{corpus1b} The boy liked the pizza.
\item \label{corpus1c} Mary thinks that the girl saw the dog.
\item \label{corpus1d} Alison says that Mary thinks that the girl saw the dog.
\item \label{corpus1e} Mary's friend's neighbor's dog wanted the pizza.
\item \label{corpus1f} The girl saw the dog by the cat near the elephant.
\end{examples}
\end{myexample}
...
A Lexicon T:
\begin{verbatim}
Name → Mary, Alison, George, Yuhan, Yamato
N → neighbor, friend, woman, man, girl, boy, cat, dog, bird, elephant, pizza
Det → the, some, this, a
V_NP → saw, liked, chased, wanted
V_S → thinks, believes, says
Prep → in, by, near, beside, on
\end{verbatim}
\end{myexample}
...
In phrase-structure terms, a category B is said to be \textit{embedded} within another category A if B combines with some other category to make A: this is simple compositionality within phrase structure.  The determiner (Det) \textit{the} can combine with the noun (N) \textit{girl} (accordong to rule 3) in order to form an NP that generates the string \textit{the girl}. The categories Det and N are then \textit{embedded} within the category NP.
...
If the rules of the grammar are such that the category A can dominate another node of category A, then we have \textit{self-embedding} of A within A. The grammar in \exref{cfg1} has three instances of self-embedding: The category S expands to another S, through rules 1 and 6: an S expands to an NP and a VP in rule 1; and a VP expands to a V$_{S}$ and then a further S category in rule 6. We can generate arbitrarily long sentences using these rules, as \cite{hauser2002faculty} indirectly suggest. Parses for self-embedded examples like \exref{corpus1c} and \exref{corpus1d} according to the above grammar are provided below:
...
The grammar in \exref{cfg1} allows two other kinds of self-embedding. The first is self-embedding of a noun phrase within a noun phrase for possessive (genitive) noun phrases, as in \exref{corpus1e} \textit{Mary's friend's neighbor's dog wanted the pizza}, whose structure is presented below:

\begin{center}
\Tree [.S [.NP [.NP [.NP [.NP [.Name Mary ] ] 's [.N friend ] ] 's [.N neighbor ] ] 's [.N dog ] ] [.VP [.\[V_{NP}\] wanted ] [.NP [.Det the ] [.N pizza ] ] ] ]

\end{center}

Here we have the NP \textit{Mary} embedded within the NP \textit{Mary's friend}, which is itself within the NP \textit{Mary's friend's neighbor}, etc.

Finally, the grammar in \exref{cfg1} allows self-embedding of VPs within other VPs by adding prepositional phrases (PPs) according to rule 8. So the structure generated by this grammar for sentence \exref{corpus1f} is provided below:

\begin{center}
\Tree [.S [.NP [.Det the ] [.N girl ] ] [.VP [.VP [.VP [.\[V_{NP}\] saw ] [.NP [.Det the ] [.N dog ] ] ] [.PP [.Prep by ] [.Det the ] [.N cat ] ] ] [.PP [.Prep near ] [.Det the ] [.N elephant ] ] ] ] 

\end{center}

When the grammar rules allow unbounded connections on the right, we refer to these structures as right-branching, such as in rule 8, and the combination of rules 1 and 6. When the rules allow unbounded connections on the left, as in rule 4, we refer to these structures as left-branching. And when the rules allow for unbounded structure in the center of a string, we refer to such structures as center-embedded (but the current grammar does not have such rules).
